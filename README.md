<p align="center">
 <img width="110px" src="https://res.cloudinary.com/anuraghazra/image/upload/v1594908242/logo_ccswme.svg" align="center" alt="Cyber Soldier" />
 <h2 align="center">⚔️ Pranaw N N</h2>
</p>
### 👋 Hi, I am Pranaw N N
**Cybersecurity Student | Blue Team Defender | Future Penetration Tester**
<a href="https://www.linkedin.com/in/pranaw-nn/">
  <img align="left" alt="Pranaw's LinkedIn" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/linkedin.svg" />
</a>
<br />
<br />
---
## 🛡️ About Me

I am a **2nd year CSE (Cyber Security) student at Karpagam College of Engineering**.  
I operate in the space between **attack and defense** — learning how systems break, so I can build them stronger.

I am an **active OWASP community member**, regularly participating in **CTFs, workshops, and hackathons**.  
My long-term mission is to become a **professional penetration tester and blue-team security analyst**, working on real-world threats across networks, cloud, and enterprise systems.

I’m deeply interested in:
- Network Security & Blue Team Operations  
- Bug Bounty & Web Exploitation  
- Reverse Engineering & OSINT  
- Cloud Security (AWS)

I’m always building, breaking, testing, and improving.

---

## ⚔️ Cyber Domains I Train In

- 🕵️ Penetration Testing & Bug Bounty  
- 🌐 Network Security  
- 🔍 OSINT & Reconnaissance  
- 🧬 Reverse Engineering  
- ☁️ Cloud Security (AWS)  
- 🛡️ Blue Team & Threat Detection

---

## 🧰 Tools & Tech Arsenal

**Languages:**  
`Python` `C` `C++` `Java` `JavaScript` `Bash`

**Pentest / Security:**  
`Nmap` `Burp Suite` `Metasploit` `Wireshark` `Nikto`

**Platforms:**  
`Linux` `Kali Linux` `Parrot OS` `AWS`

**Blue Team / Monitoring:**  
`Splunk`

---

## 🧪 Experience & Activity

- OWASP Community Member  
- Active CTF Participant  
- Cybersecurity Workshops & Hackathons  
- Exploring Bug Bounty & Cloud Security  
- Student Developer & System Builder

---

## 📌 Featured Projects


[![ReadMe Card](https://github-readme-stats.vercel.app/api/pin/?username=pranaw-nn&repo=zero-based-inbox-processor)](https://github.com/pranaw-nn/zero-based-inbox-processor)
[![ReadMe Card](https://github-readme-stats.vercel.app/api/pin/?username=pranaw-nn&repo=IoT-Device-Monitoring-System)](https://github.com/pranaw-nn)
[![ReadMe Card](https://github-readme-stats.vercel.app/api/pin/?username=pranaw-nn&repo=paperboy-cms)](https://github.com/pranaw-nn)
[![ReadMe Card](https://github-readme-stats.vercel.app/api/pin/?username=pranaw-nn&repo=todo-list-app)](https://github.com/pranaw-nn)

---

## 📊 War Stats

![Pranaw's GitHub stats](https://github-readme-stats.vercel.app/api?username=pranaw-nn&show_icons=true&theme=radical)

---

<details>
<summary>⚙️ Current Focus</summary>

- Strengthening networking and blue team foundations  
- Building security-focused tools and systems  
- CTFs, bug bounty practice, and cloud security labs  
- Preparing for advanced penetration testing

![picture](https://raw.githubusercontent.com/saadeghi/saadeghi/master/dino.gif)
</details>

---

🔥 *Training today. Defending tomorrow.*
